...
local LavaCave = Map:extend("LavaCave")

function LavaCave:makeTileMap()
  local params = self:generateParams()
  return self:makeLavaCave(params)  -- Lightmap isn't really a function of map, but it's faster this way
end

function LavaCave:generateParams()
...
function LavaCave:makeLavaCave(genParams)
  local lightMap = {}
  local map = {}
...
function LavaCave:determineTile(x, y, genParams)
